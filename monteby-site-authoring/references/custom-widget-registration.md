...
Monteby child-theme widgets are declared in PHP and rendered by WordPress. The host-widget model is `schema + defaults + aiProps + PHP render_callback`; it does not require React editor components or a browser compiler entry.

Product widgets shipped by Monteby Builder/Core are different: they require
editor, JavaScript compiler, PHP renderer, Core renderer, schema, AI contract,
and parity coverage. A child theme or site plugin must not duplicate that
product surface merely to register a host-specific widget.
...
Before authoring or saving a host widget, require all of the following from the
live contract:

- `productVersion` satisfies the `providerRenderedWidgetSave` feature gate from
  `site-contract-compatibility.json`;
- `authoring.capabilities.providerRenderedWidgetSave` is exactly `true`;
- the widget's live component entry contains its schema, defaults, `aiProps`,
  placement rules, and provider-rendered classification.

Canonical save HTML is generated by the Builder's PHP renderer. Do not call a
JavaScript compiler for a provider-rendered widget and do not submit authored
HTML with the layout. An older product is `blocked_plugin_version`; a
new-enough product missing the capability is `blocked_contract_inconsistency`.
An unknown widget remains a validation failure. A callback exception may produce
only the Builder's sanitized failure marker; it is not permission to inject raw
HTML or bypass validation.

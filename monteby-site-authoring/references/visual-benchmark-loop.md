...
discriminator the document validates cleanly and paints nothing.
   ...
   When no live WordPress/Builder preview URL exists yet, use `scripts/render-monteby-preview.js --contract contract.json --layout candidate/layout-draft.json --out candidate/layout-draft-preview.html --fragment-out candidate/layout-draft-fragment.html` with the same full live contract used for drafting as a temporary local diagnostic renderer, serve that directory, and capture it with `capture-template-reference.js --capture-layout --full-page --name candidate`. Published global bindings and font origins must resolve; missing references or font sources are blockers, not permission to substitute a palette or font. This static preview is useful for fast screenshot loops, but it is not canonical proof; final validation must still use the WordPress/PHP renderer from Monteby Builder + Monteby Theme.
   ...
   For an owned raster reference (a screenshot the user holds rights to, or an image generated locally by a design skill from the project's own brief and tokens), do not force it through the marketplace research path: follow `references/design-handoff.md`, turn the image into owned local HTML, and continue in `owned-html-reconstruction` with a `handoff.json` provenance record.
...
4. Build the Monteby node map.
   ...
   - `paddingTop`, `paddingBottom`, `innerMaxWidth`, `innerPaddingX`, or a matching `sectionRhythmPreset`/`sectionContentWidthPreset` when the measured rhythm/width matches a preset
   - `layoutDisplay`, `flexDirection`, `flexWrap`, `justifyContent`, `alignItems`, `gap`, `rowGap`, `columnGap`
   - `background`, `backgroundType`, gradient/image props, `backgroundLayers`, and background-video props when exposed
   ...
   Since Builder 1.2.0 the following measured behaviors are expressible with typed controls and must not be degraded to product gaps when the live contract exposes them:

   | Measured behavior | Composition since 1.2.0 |
   |---|---|
   | Multi-stop gradient (3 to 5 stops) | `backgroundLayers` item with `type: "linear"`, `color1..color5`, `color1Stop..color5Stop` |
   | Positioned radial glow | `backgroundLayers` item with `type: "radial"`, `positionX`/`positionY`, `size` |
   | Elliptical radial | Same layer with `shape: "ellipse"` and `sizeY` (plus responsive variants) |
   | Dimmed/tinted media frame | `backgroundMediaBrightness`/`Saturate`/`Contrast` with `Tablet`/`Mobile` tiers |
   | Background video | `backgroundVideo` with required `backgroundVideoPoster`, `backgroundVideoFit`, and explicit `backgroundVideoMobileBehavior` |
   | Uniform band rhythm matching a preset | `sectionRhythmPreset` (`compact`/`standard`/`spacious`/`hero`) with responsive variants |
   | Sibling overlap/pull-up | `Container.layerPull` plus `layerIndex`/`paintLayer` |
   | Linked heading | `Heading.href` or `Heading.dynamicHref` |
   | Multi-step form | FormBlock v2: `fieldId`, `steps`, `progress`, `visibleWhen`, attribution and collection-notice props |

   The product-gap rule remains for what the contract still cannot express, for example conic gradients, `url()` layers inside a gradient stack, `var()`/`calc()` values, and more than five stops in one layer.
   ...
   When a target uses layered gradient artwork, soft radial color spots, or an accent glow over a main gradient, first check the live `Section`/`Container` contract for `backgroundLayers`. On a 1.2.0 contract, model each measured layer as one `backgroundLayers` item, top layer first: multi-stop linears carry `color1..color5` with `color1Stop..color5Stop`, radial spots carry `positionX`/`positionY`/`size` (and `shape: "ellipse"` plus `sizeY` for ellipses), and each layer can set `opacity` and `blendMode` from the listed options. On an older contract without `backgroundLayers`, keep the main layer in `backgroundType: "gradient"` plus `gradient*` props and add `backgroundAccentType: "radial"` with `backgroundAccentColor1`, `backgroundAccentColor2`, `backgroundAccentPositionX`, `backgroundAccentPositionY`, and `backgroundAccentSize`. Do not approximate layered backgrounds with pseudo-elements, raw CSS, utility classes, or extra decorative containers unless the contract lacks both routes.
   ...
   When a marketplace-style hero visual uses multiple independent radial glows over one panel, prefer `backgroundLayers` on a 1.2.0 contract: one radial layer per glow with its own `positionX`/`positionY`/`size`/`opacity`, above the base linear layer. On an older contract, check whether the live contract exposes `backgroundAccent2Type` plus `backgroundAccent2Color1`, `backgroundAccent2Color2`, `backgroundAccent2PositionX`, `backgroundAccent2PositionY`, and `backgroundAccent2Size`. If exposed, map the primary/top glow to `backgroundAccent*`, the secondary glow to `backgroundAccent2*`, and preserve the main linear gradient as the base background layer. If neither `backgroundLayers` nor the second accent controls exist, map only the strongest visible glow and record a Builder/Core follow-up instead of adding raw CSS, pseudo-elements, duplicate decorative nodes, or utility classes.

# QUARANTINED — unsafe parallel draft, not skill instructions

This file is preserved only as audit evidence. It must not be executed or linked
from `SKILL.md`: it exposes credentials in argv, bypasses the canonical state
machine, permits blind conflict retries, accepts partial geometry, and treats
residual CSS as completion.
...
## Krok 4 — Kompilacja rusztowania (2 min)

Nie piszesz layoutu od zera. Najpierw pełna specyfikacja makiety:

```bash
node $SKILL/scripts/extract-reference-spec.mjs --emit-snippet
```

Wykonaj snippet w przeglądarce na makiecie przy szerokości **1440**, zapisz wynik
jako `.monteby/spec-1440.json`. Potem kompilacja:

```bash
node $SKILL/scripts/spec-to-layout.mjs --contract .monteby/contract.json \
  --spec .monteby/spec-1440.json --out .monteby/layout.json \
  --report .monteby/compile-report.json
```

Kompilator przenosi strukturę, treść, typografię, siatki, tła i formularze
mechanicznie (z automatyczną zgodą RODO, wariantami responsywnymi siatek
i naprawą pułapek renderera). Kalibracja: ~88% głębokości makiety bez żadnej
ręcznej pracy.

**Warunek:** kompilacja kończy się bez wyjątku i raport istnieje.

Twoja sprawczość zaczyna się w raporcie: to zamknięta lista kwestii, każda
z adresatem (`media`, `manual-controls`, `dropped-prop`, `child-theme`, …).
Procedury per adresat są w `references/mechanical-pipeline-audit.md` —
przepracuj listę **przed** pomiarem, bo część pozycji (podmiana mediów) zmienia
geometrię. Poprawki nanoś edytując `layout.json` przez `layout-kit.mjs` lub
wprost w JSON — po każdej rundzie wróć do kroku 5.

Gdy makieta jest tylko obrazem (bez HTML), dopiero wtedy buduj ręcznie na bazie
`layout-kit.mjs` według tabel z kroku 3 i `references/html-to-monteby.md`.
...
Czytaj wynik mechanicznie (pełne procedury per objaw:
`references/mechanical-pipeline-audit.md`):
...
3. Resztę — **wyłącznie po wyraźnej, imiennej zgodzie właściciela serwisu**
   (polityka: `child-theme-residual-styles.md`; domyślnie residualny CSS jest
   zabroniony) — zapisz jako plan i wygeneruj arkusz:

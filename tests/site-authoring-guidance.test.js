...
test('skill gates provider-rendered host widgets and separates them from product widgets', () => {
  const customWidgets = read(path.join(references, 'custom-widget-registration.md'));
  assert.match(skill, /providerRenderedWidgetSave: true/);
  assert.match(skill, /blocked_plugin_version/);
  assert.match(skill, /blocked_contract_inconsistency/);
  assert.match(customWidgets, /Product widgets shipped by Monteby Builder\/Core/);
  assert.match(customWidgets, /Canonical save HTML is generated by the Builder's PHP renderer/);
...
test('subpixel and source-text policies preserve strict verdict semantics', () => {
  ...
  assert.match(skill, /--subpixel-authorization FILE/);
  assert.match(skill, /This verdict is never 1:1/);
  assert.match(skill, /`missing`, `truncatedOrChanged`, or\s+`duplicateCountMismatch`/);
